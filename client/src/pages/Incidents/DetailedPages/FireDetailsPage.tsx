...
const FireDetailsPage: FC = () => {
  ...
  const [fireIncident, setFireIncident] = useState<Incident>();
  const [loading, setLoading] = useState(true);
    ...
    useEffect(() => {
      const fetchFireLogs = async () => {
        ...
        try {
          const resp = await instance.get(`/incident/${fireId}`);
          console.log("Success")
          console.log(resp.data)
          console.log("Get Severity: ", resp.data.severity)
          if (resp.data != null) {
            setFireIncident(resp.data)
            console.log("Fire Incident Severity: ", fireIncident?.severity)
          } else {
            console.log(resp.data);
          }
        }
    ...
    <div className='flex'>
        ...
        <div className='ml-auto w-5/6 text-4xl px-4 pt-2 gap-y-10 justify-evenly items-center'>
            ...
            <div className='text-2xl flex mt-10 justify-between'>
                <div>Severity:</div>
                <div className="flex"> 
                  <StyledRating name="customized-10 border-1" value={fireIncident?.severity ? Number(fireIncident.severity) : 1} max={3} readOnly />
                </div>
            </div>
            <div className='text-2xl flex mt-10 justify-between'>
                <div>When it happened:</div>
                <div className="flex"> 
                  <p>{fireIncident?.timestamp.toString()}</p>
                </div>
            </div>
            <div className='text-2xl flex mt-10 justify-between'>
                <div>Status:</div>
                <div className="flex"> 
                  <p>{fireIncident?.status}</p>
                </div>
            </div>
            <div className='text-2xl flex mt-10 justify-between'>
                <div>Which camera detected:</div>
                <div className="flex"> 
                  <p>{fireIncident?.detected_by_camera.location}</p>
                </div>
            </div>
            <div className='text-2xl flex mt-10 justify-between'>
                <div>Which person was responsible:</div>
                <div className="flex"> 
                  <p>{fireIncident?.worker.full_name}</p>
                </div>
            </div>
            <div className='text-2xl flex mt-10 justify-between'>
                <div>Which department:</div>
                <div className="flex"> 
                  <p>{fireIncident?.worker.department}</p>
                </div>
            </div>
        </div>
    </div>
